...
router.post("/", async (req, res) => {
  try {
    Event.create(req.body).then((event) => res.status(201).json({ status: 201, event: event }));
  } catch (error) {
...
router.get("/", async (req, res) => {
  try {
    Event.find().then((posts) => res.json(posts));
  } catch (error) {
...
router.get("/:id", async (req, res) => {
  try {
    const { id } = req.params;
    Event.findById(id).then((post) => res.json(post));
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
});
...
router.route("/add/:id").put(async (req, res) => {
  try {
    ...
    const addUserEvent = await eventDoc.updateOne({ $addToSet: { users: email } });
